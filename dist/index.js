...
function main() {
  return __async(this, null, function* () {
    ...
    const path = (0, import_core.getInput)("path") || ".env.production";
    const ignoredEnvs = String((0, import_core.getInput)("ignore-env") || "").split(",").map((env) => env.trim()).filter((env) => env.length > 0);
    ...
    if (file) {
      const dotenv = import_dotenv.default.parse(file);
      const expanded = (0, import_dotenv_expand.expand)(dotenv);
      if (expanded) {
        console.log(`setting output and env for ${Object.keys(expanded).join(", ")}`);
        for (const [key, value] of Object.entries(expanded)) {
          if (ignoredEnvs.includes(key))
            continue;
          (0, import_core.setOutput)(key, value);
